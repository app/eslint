    ...
    create(context) {
        ...
        if (context.options.length) {
            ...
            if (context.options[1]) {
                ...
                if (typeof opts.ArrayExpression !== "undefined") {
                    options.ArrayExpression = opts.ArrayExpression;
                }

                if (typeof opts.ObjectExpression !== "undefined") {
        ...
        */
        function getDesiredIndent(token) {
            if (ignoredTokens.has(token)) {
                ...
                return getTokenIndent(token).goodChar / indentSize;
            }
            const offsetInfo = desiredOffsets.get(token);

            return offsetInfo.offset + (offsetInfo.from ? getDesiredIndent(offsetInfo.from) : 0);
        }
        ...
        */
        function getTokensAndComments(node) {
            ...
            if (!tokensByNodeCache.has(node)) {
                ...
                const nodeTokens = parentTokens.filter(token => token.range[0] >= node.range[0] && token.range[1] <= node.range[1]);
        ...
        */
        function addFunctionParamsIndent(node, paramsIndent) {
            ...
            const paramTokens = getTokensAndComments(node).filter(token => token.range[0] >= openingParen.range[0] && token.range[1] <= closingParen.range[1]);
        ...
        */
        function addBinaryOrLogicalExpressionIndent(node) {
            ...
            const tokensAfterOperator = tokens.filter(token => token.range[0] >= firstTokenAfterOperator.range[0]);
        ...
        return {
            ...
            ImportDeclaration(node) {
                if (node.specifiers.filter(specifier => specifier.type === "ImportSpecifier").length) {
                    ...
                    const openingCurly = allTokens.find(token => token.value === "{");
                    const closingCurly = allTokens.find(token => token.value === "}");
                    const specifierTokens = allTokens.filter(token => token.range[0] >= openingCurly.range[0] && token.range[1] <= closingCurly.range[1]);
            ...
            MemberExpression(node) {
                ...
                const tokensToIndent = tokens.filter(token => token.range[0] >= firstNonObjectToken.range[0]);
            ...
            SwitchStatement(node) {
                ...
                const casesWithBlocks = new WeakSet(node.cases.filter(switchCase => switchCase.consequent.length === 1 && switchCase.consequent[0].type === "BlockStatement").map(switchCase => sourceCode.getFirstToken(switchCase)));
